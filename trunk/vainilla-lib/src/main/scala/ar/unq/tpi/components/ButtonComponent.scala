...
class ButtonComponent[SceneType <: EventGameScene](sprite: Sprite, over: Sprite, x: Double, y: Double, listener:(DeltaState)=>Unit) extends SpriteComponent[SceneType](sprite, x, y) {
  
  override def setScene(scene:SceneType){
    scene.addMousePressetListener(this, MouseButton.LEFT, onClick)
  }
...
class ButtonTextComponent[SceneType <: EventGameScene](sprite: Sprite, over: Sprite, label: Label, x: Double, y: Double, listener:(DeltaState)=>Unit) extends 
																						ButtonComponent[SceneType](sprite, over, x, y, listener) {
