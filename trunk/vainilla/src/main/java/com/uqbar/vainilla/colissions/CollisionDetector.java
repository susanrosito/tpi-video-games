...
public class CollisionDetector {
	...
	public boolean collidesRectAgainstRect(final GameComponent<?, ? extends Appearance> target, final List<GameComponent<?, ? extends Appearance>> posibles){
		for (GameComponent<?, ? extends Appearance> gameComponent : posibles) {
			if(collidesRectAgainstRect(target, gameComponent)){
				return true;
			}
		}
		return false;
	...
	public boolean collidesRectAgainstRect(final GameComponent<?, ? extends Appearance> target, final GameComponent<?, ? extends Appearance> comp){
		return this.collidesRectAgainstRect(target.getX(), target.getY(), (int)target.getAppearance().getWidth(), (int)target.getAppearance().getHeight(), 
				comp.getX(), comp.getY(), (int)comp.getAppearance().getWidth(), (int)comp.getAppearance().getHeight());
