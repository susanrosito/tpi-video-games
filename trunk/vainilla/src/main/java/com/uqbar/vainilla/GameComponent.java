...
public class GameComponent<SceneType extends GameScene, A extends Appearance> implements Comparable<GameComponent<?, ?>>{
	...
	public GameComponent() {
		this((A) new Invisible(), 0, 0);
	}

	public GameComponent(double x, double y) {
		this((A) new Invisible(), x, y);
	}

	public GameComponent(A appearance, double x, double y) {
	...
	public void alignCloserBoundTo(GameComponent<?, ? extends A> target) {
		A ownBounds = this.getAppearance();
		A targetBounds = target.getAppearance();
	...
	public int compareTo(GameComponent<?, ?> o) {
		return Integer.valueOf(this.z).compareTo(o.getZ());
	}
