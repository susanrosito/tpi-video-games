...
class Hud(scene: GamePlayScene) {
  ...
  val bar1 = new LifeBar(GameImage.LIFE_BAR.flipHorizontally(), GameImage.BACKGROUND_BAR.flipHorizontally(), GameImage.HUD_BAR_1.flipHorizontally(), GameImage.HUD_BAR_2.flipHorizontally(), scene.character1.character.envelope.selectedImage.crop(50, 30, 150, 190).scale(80D / 100, 100D / 160), true, (scene.getGame().getDisplayWidth() / 2) - GameImage.BACKGROUND_BAR.getWidth() + 30, 0, scene.character1.character.getLife)
  var bar2 = new LifeBar(GameImage.LIFE_BAR, GameImage.BACKGROUND_BAR, GameImage.HUD_BAR_1, GameImage.HUD_BAR_2, scene.character2.character.envelope.selectedImage.crop(50, 30, 150, 190).scale(80D / 100, 100D / 160), false, bar1.getX() + GameImage.BACKGROUND_BAR.getWidth() - 30, 0, scene.character2.character.getLife)
